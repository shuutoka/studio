...
export function MediaGallery({ projects, onOpenProject, onOpenCharacter, onLinkCharacterImage }: { projects: StudioProject[]; onOpenProject?: (project: StudioProject) => void; onOpenCharacter?: (project: StudioProject, characterId: string) => void; onLinkCharacterImage?: (projectId: string, characterId: string, mediaId: string, linked: boolean) => void }) {
          ...
          <div className={layout === "list" || layout === "text" ? "grid gap-3" : layout === "images" ? "grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5" : "grid gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4"}>
            {filtered.map((item) => (
              <article key={`${item.media.id}-${item.characterId}-${item.outfitName ?? "portrait"}`} className={`overflow-hidden rounded-2xl border border-white/8 bg-[#131218] ${layout === "list" ? "grid grid-cols-[140px_1fr]" : ""}`}>
                {layout !== "text" && <MediaPreview mediaId={item.media.id} alt={item.outfitName ? `${item.characterName} — ${item.outfitName}` : item.characterName ?? item.media.name} className={layout === "list" ? "h-full min-h-28 w-full rounded-none" : "aspect-[4/3] w-full rounded-none"} expandable gallery={lightboxGallery} fit={thumbnailFit} characterLinking={onLinkCharacterImage ? { projectId: item.project.id, projectName: item.project.name, characters: item.project.characters.map((character) => ({ id: character.id, name: character.name, imageIds: character.imageIds })), getContext: (mediaId) => { const context = contexts.find((candidate) => candidate.media.id === mediaId) ?? item; return { projectId: context.project.id, projectName: context.project.name, characters: context.project.characters.map((character) => ({ id: character.id, name: character.name, imageIds: character.imageIds })) }; }, onChange: (mediaId, targetCharacterId, linked, targetProjectId) => onLinkCharacterImage(targetProjectId ?? item.project.id, targetCharacterId, mediaId, linked) } : undefined} />}
