...
import {
  getManuscriptFilename, getManuscriptPageCount,
} from "@/lib/writing-export";
import { exportNativeWriting, type NativeWritingExportFormat } from "@/lib/writing-native-export";
...
export function WritingExportButton({
  ...
  const [format, setFormat] = useState<NativeWritingExportFormat>("docx");
  ...
  async function startExport() {
    ...
    try {
      const result = await exportNativeWriting(project, selectedVolumeId, format);
      setOpen(false);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="border-white/10 bg-[#17151d] text-[#eeeaf2]">
        <DialogHeader>
          ...
          <DialogDescription className="text-[#9c96a5]">Le DOCX est exporté directement depuis le moteur natif. Le PDF et l’impression reprennent les pages visibles dans l’espace Écriture.</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4">
          ...
          <label className="grid gap-2 text-xs font-medium text-[#aaa4b4]">Format du document
            <Select value={format} onValueChange={(value: NativeWritingExportFormat) => setFormat(value)}>
